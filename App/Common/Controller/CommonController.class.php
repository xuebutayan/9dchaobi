...
class CommonController extends Controller {
    ...
    public function _initialize(){
        ...
        if(!$this->config['site_status']) $this->temp_close();//临时关闭前台指向
		//分红奖励控制参数
    ...
    function temp_close(){
        if(MODULE_NAME =='Home' && CONTROLLER_NAME !='Api'){
    ...
    *  /**
    ...
    //修正众筹表 计算剩余数量  修改状态
     ...
     //积分日志
     protected function inte_log($member_id,$num,$type=0,$title='积分交易'){
        $log = M('integrals_log');
        $data=[
            'member_id'=>$member_id,
            'title'=>$title,
      ...
      */
      protected   function setUserMoney($member_id,$currency_id,$num,$inc_dec,$field){
         ...
         if ($currency_id==0){
            ...
            //修正字段
             switch ($field){
                 case 'forzen_num': $field='forzen_integrals';break;
                 case 'num': $field='integrals';break;
             }
             switch ($inc_dec){
                 case 'inc':
                     $msg= M('Member')->where("member_id=$member_id ")->setInc($field,$num);
                     if($field=='integrals'){
                        $re = $this->inte_log($member_id,$num,1);
                        $msg = $msg && $re;
                     }
                     break;
                 case 'dec':
