...
class TradeController extends TradeFatherController
{
    ...
    private function checkUserMoney($num, $price, $type, $currency)
    {
        ...
        //和自己的账户做对比 获取账户信息
        if($_POST['jifen']){
            $money = M('Member')->where(array('member_id'=>$_SESSION['USER_KEY_ID']))->getField('integrals');
        }else $money = $this->getUserMoney($currency_id, 'num');
     ...
     * @param char $type 买buy 卖sell
     * @param $currency_id 交易币种
     * @param int $jifen 1为积分购买
     */
    private function guadan($num, $price, $type, $currency,$jifen=0)
    {
        //获取交易币种信息
        switch ($type) {
            case 'buy':
        ...
        $data = array(
            ...
            'currency_id'       => $currency['currency_id'],
            'currency_trade_id' => $jifen,//$currency['trade_currency_id'],
            'price'             => $price,
            'num'               => $num,
            'trade_num'         => 0,
            'fee'               => $fee,
    ...
    {
        switch ($type) {
            case 'buy':$fee = $num * $fee;
                break;
            case 'sell':$fee = $price * $num * $fee;
                break;
            default:$fee = 0;
        }
