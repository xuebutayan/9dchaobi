...
class PendingController extends AdminController {
	...
	public function index() {
		...
		if (!empty($user_name))
			$where['m.user_name'] = array('LIKE', "%{$user_name}%");
		if (!empty($status))
		...
		$info = $withdraw
					->table('__WITHDRAW__ w')
					->field('w.*,b.cardname,b.bank_branch,b.cardnum,b.bankname,ab.area_name barea_name,ab.parent_id,a.area_name aarea_name,m.nick,m.email,m.user_name')
					->join('__BANK__ b ON w.bank_id=b.id', 'LEFT')
